<x-pondit-limitless-master>
    ...
    <x-pondit-pl-card cardTitle="PURCHASE ORDER AND BUDGET COMPLETE LIST">
        <x-pondit-batable tableId="baTable" class="table">
            @php
                ...
                $user_roll = DB::table('system_role_user')
                            ->join('system_roles', 'system_role_user.role_id', 'system_roles.id')
                            ->where('system_role_user.user_id', auth()->user()->id)->first();
            @endphp
            <x-slot name="thead">
                <tr>
            ...
            @foreach ($purchase_requisitions as $key => $purchase)
                <tr class="text-center clickable-tr" data-id="{{$purchase->id}}">
                        ...
                        <td class="text-center ">
                            ...
                            @if (!is_null($purchase->authorized_at) || !is_null($purchase->accounts_verified_at))
                                <x-pondit-pl-btn icon="file" color="success"
                                    href="{{ route('purchase.po_budget_pdf', $purchase->id) }}" tooltip="PDF"></x-pondit-pl-btn>
                            @endif
                        </td>
                    @else
                    ...
                    <td class="text-center ">
                        ...
                        <x-pondit-pl-btn icon="tv" color="success"
                        href="{{ route('purchase.po_budget_preview', $purchase->id) }}" tooltip="preview"></x-pondit-pl-btn>
                  ...
                  @endif
                    ...
                    <td>{{ $purchase->authorized_at ? date('d-M-Y', strtotime($purchase->authorized_at)):''}}</td>
                        ...
                        @endif
                    <td>{{ number_format($purchase->total, 2, '.', ',') ?? 0.00 }}</td>
                        @php
